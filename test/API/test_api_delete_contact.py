...
def test_successful_contact_deletion(contact_setup):
    header, cont_id = contact_setup
...
def test_delete_contact_already_deleted(contact_setup):
    header, cont_id = contact_setup
    contact_api = ContActsApi()
    delete_resp = contact_api.delete_contact(header, cont_id)
    assert delete_resp.status_code == 200, f"Expected 200, got {delete_resp.status_code}"
...
def test_delete_contact_with_invalid_id(auth_header):
    contact_api = ContActsApi()
...
def test_try_update_deleted_contact(auth_header):
    contact_api = ContActsApi()
